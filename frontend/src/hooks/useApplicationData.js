...
const useApplicationData = () => {
  function reducer(state, action) {
    switch (action.type) {
      case ACTIONS.FAV_PHOTO_ADDED:
        return {
          ...state,
          likePhotoArray: [action.id, ...state.likePhotoArray],
        };
      case ACTIONS.FAV_PHOTO_REMOVED:
        return {
          ...state,
          likePhotoArray: state.likePhotoArray.filter((id) => id !== action.id),
        };
  ...
  useEffect(() => {
    if (state.topicId !== null) {
      fetch(`http://localhost:8001/api/topics/${state.topicId}/photos`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((data) => dispatch({ type: "SET_PHOTO_DATA", payload: data }))
